...
seurat_obj <- data1
##########################################################
ref_centroids <- sapply(unique(bulk_labels), function(g) {
  rowMeans(ref_mat[, bulk_labels == g, drop = FALSE])
})
test_mat <- as.matrix(test_mat)
mode(test_mat) <- "numeric"
# 运行 cluster reproducibility
repro_results <- clusterRepro(
...
# 将cluster映射回单细胞
seurat_obj$clusterRepro_label <- cluster_assign[seurat_obj$seurat_clusters]
